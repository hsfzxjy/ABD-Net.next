...
from torchreid.data_manager_aicity19_crop_n import AICity19ImageDataManagerCropN
...
def main():
    ...
    dm = AICity19ImageDataManagerCropN(use_gpu, crop_n=5, **image_dataset_kwargs(args))
    ...
    def eval_set(name, loader):
        ...
        with torch.no_grad():
            ...
            for batch_idx, package in enumerator:
                ...
                if len(imgs) == 5:
                    features = torch.cat(get_dim_4_feature(model, img_batch) for img_batch in imgs) / len(imgs)
                else:
