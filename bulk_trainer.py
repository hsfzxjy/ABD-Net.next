...
def get_next_available_gpu() -> Optional[int]:
    ...
    gpus = GPUtil.getAvailable(maxMemory=0.4, excludeID=[used_gpu for used_gpu, count in used_gpus.items() if count >= 1])
    if not gpus:
        return None
    return gpus[0]
            ...
            for process, args, p_gpu in processes:
                ...
                try:
                    ...
                    if process.returncode != 0:
                        print(f'FAILED: {args["log_dir"]} with code {process.returncode}')
