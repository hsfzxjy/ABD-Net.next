...
def test_classification(model, valloader, use_gpu):
    ...
    with torch.no_grad():
        for imgs, pids, _, _ in valloader:
        ...
        final_acc = accuracy(sum(final_feature), target)[0].item()
        print('Feature Final ACC:', final_acc)
        return final_acc


def test_reid(model, queryloader, galleryloader, use_gpu, ranks=[1, 5, 10, 20], return_distmat=False):
