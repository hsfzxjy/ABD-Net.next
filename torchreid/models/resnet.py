...
class ResNetTr9(nn.Module):
    ...
    def __init__(self, num_classes,
        ...
        if os.environ.get('non_local') is not None:
            ...
            self.non_local = NONLocalBlock2D(2048)
...
class ResNetTr10(nn.Module):
    ...
    def __init__(self, num_classes,
        ...
        # self.inplanes = 64
        super(ResNetTr10, self).__init__()
        self.feature_dim = 2048  # 512 * block.expansion
...
class ResNetTr11(nn.Module):
    ...
    def __init__(self, num_classes,
        ...
        # self.inplanes = 64
        super(ResNetTr11, self).__init__()
        self.feature_dim = 2048  # 512 * block.expansion
    ...
    def get_tricky_8_attention_module(self):
        ...
        for p in range(1, self.part_num + 1):
            ...
            before = DANetHead(in_channels, out_channels, nn.BatchNorm2d, lambda _: lambda x: x)
            cam = DANetHead(in_channels, out_channels, nn.BatchNorm2d, CAM_Module)
            pam = DANetHead(in_channels, out_channels, nn.BatchNorm2d, PAM_Module)
            sum_conv = nn.Sequential(nn.Dropout2d(0.1, False), nn.Conv2d(out_channels, out_channels, 1))

            setattr(self, f'before_module{p}', before)
            self._init_params(before)
            setattr(self, f'pam_module{p}', pam)
            self._init_params(pam)
            setattr(self, f'cam_module{p}', cam)
            self._init_params(cam)
            setattr(self, f'sum_conv{p}', sum_conv)
            self._init_params(sum_conv)
    ...
    def forward(self, x):
        ...
        for p in range(1, self.part_num + 1):
            ...
            f_before1 = getattr(self, f'before_module{p}')(f)
            f_cam1 = getattr(self, f'cam_module{p}')(f)
            f_pam1 = getattr(self, f'pam_module{p}')(f)

            f_sum1 = f_cam1 + f_pam1 + f_before1
            f_after1 = getattr(self, f'sum_conv{p}')(f_sum1)
...
class ResNetTr12(nn.Module):
    ...
    def __init__(self, num_classes,
        ...
        # self.inplanes = 64
        super(ResNetTr12, self).__init__()
        self.feature_dim = 2048  # 512 * block.expansion
...
for fragment in fragments:
    ...
    make_function_sf_tricky_50(name, config, 9, base_class=ResNetTr9)

for fragment in fragments:

    name = f'resnet50_sf_tr10'
    config = {}
    for key, (sub_config, name_frag) in zip(keys, fragment):
        name += name_frag
        config.update({key: sub_config})

    make_function_sf_tricky_50(name, config, 10, base_class=ResNetTr10)

for fragment in fragments:

    name = f'resnet50_sf_tr11'
    config = {}
    for key, (sub_config, name_frag) in zip(keys, fragment):
        name += name_frag
        config.update({key: sub_config})

    make_function_sf_tricky_50(name, config, 11, base_class=ResNetTr11)

for fragment in fragments:

    name = f'resnet50_sf_tr12'
    config = {}
    for key, (sub_config, name_frag) in zip(keys, fragment):
        name += name_frag
        config.update({key: sub_config})

    make_function_sf_tricky_50(name, config, 12, base_class=ResNetTr12)
