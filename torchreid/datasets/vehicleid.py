...
class VehicleID(BaseImageDataset):
    ...
    def __init__(self, num=800, root='data', verbose=True, **kwargs):
        super().__init__()

        self.dataset_dir = osp.join(root, self.dataset_dir)
        self.num = num
        self.image_dir = osp.join(self.dataset_dir, 'image')
        self.test_info_dir = osp.join(self.dataset_dir, 'VehicleID_eval_split')
    ...
    def _get_train(self):
        ...
        ids = defaultdict(int)
        trains = []
        with open(osp.join(self.dataset_dir, 'train_test_split', 'train_list.txt')) as f:
            for line in f:
                name, vid = line.strip().split()
                name = osp.join(self.image_dir, name + '.jpg')
                vid = int(vid)
                trains.append((name, vid, ids[vid]))
                ids[vid] += 1

            mapping = {vid: label for label, vid in enumerate(sorted(ids.keys()))}
            new_trains = []
            for name, vid, cid in trains:
                new_trains.append((name, mapping[vid], cid))
